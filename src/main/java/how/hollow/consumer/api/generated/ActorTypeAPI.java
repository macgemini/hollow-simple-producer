...
public class ActorTypeAPI extends HollowObjectTypeAPI {
    ...
    public int getActorId(int ordinal) {
        if(fieldIndex[0] == -1)
            return missingDataHandler().handleInt("Favorite", ordinal, "actorId");
    ...
    public Integer getActorIdBoxed(int ordinal) {
        int i;
        if(fieldIndex[0] == -1) {
            i = missingDataHandler().handleInt("Favorite", ordinal, "actorId");
        } else {
    ...
    public int getActorNameOrdinal(int ordinal) {
        if(fieldIndex[1] == -1)
            return missingDataHandler().handleReferencedOrdinal("Favorite", ordinal, "actorName");
