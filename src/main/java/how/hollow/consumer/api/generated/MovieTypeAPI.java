...
public class MovieTypeAPI extends HollowObjectTypeAPI {
    ...
    public int getId(int ordinal) {
        if(fieldIndex[0] == -1)
            return missingDataHandler().handleInt("AlreadyApplied", ordinal, "id");
    ...
    public Integer getIdBoxed(int ordinal) {
        int i;
        if(fieldIndex[0] == -1) {
            i = missingDataHandler().handleInt("AlreadyApplied", ordinal, "id");
        } else {
    ...
    public int getTitleOrdinal(int ordinal) {
        if(fieldIndex[1] == -1)
            return missingDataHandler().handleReferencedOrdinal("AlreadyApplied", ordinal, "title");
    ...
    public int getActorsOrdinal(int ordinal) {
        if(fieldIndex[2] == -1)
            return missingDataHandler().handleReferencedOrdinal("AlreadyApplied", ordinal, "favorites");
